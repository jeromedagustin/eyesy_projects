 ...
 */
export class Drawshashmarksangledsteppedcolor implements Mode {
  private xr = 1280;
  private yr = 720;
  ...
  draw(canvas: Canvas, eyesy: EYESY): void {
    ...
    const min200 = Math.floor(this.xr * 0.156);
    const min100 = Math.floor(this.xr * 0.078);
    const max1000 = Math.floor(this.xr * 0.781);
    const max700 = Math.floor(this.yr * 0.972);
    const ran50 = Math.floor(this.xr * 0.039);
    const ran70 = Math.floor(this.xr * 0.055);
    ...
    if (this.trigger) {
      ...
      this.angle = Math.floor(Math.random() * 75);
    }
    
    for (let k = 0; k < this.vertLines; k++) {
      this.xpos = this.x + (k + 1) * (this.width / this.vertLines);
      const color = eyesy.color_picker_lfo(eyesy.knob4, 0.25); // stepped color
      canvas.line(
