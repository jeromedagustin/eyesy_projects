 ...
 * Knob4 - Visible range + trails opacity
...
export class Isometricwaverunner implements Mode {
  private xr = 1280;
  private yr = 720;
  ...
  async setup(canvas: Canvas, eyesy: EYESY): Promise<void> {
    ...
    this.patterns = this.generatePattern(75, eyesy);
  }
  ...
  draw(canvas: Canvas, eyesy: EYESY): void {
    ...
    // Process patterns - only draw visible range
    const visibleRange = 64 * eyesy.knob4;
    const minVisible = l - visibleRange - 1;
    const maxVisible = l + visibleRange + 1;
    
    for (let p = 0; p < this.patterns.length; p++) {
      if (p < minVisible || p > maxVisible) {
        continue; // Skip patterns outside visible range
      }
  ...
  private generatePattern(tileWidthHalf: number, eyesy: EYESY): Pattern[] {
    ...
    const maxRange = Math.max(1, Math.floor(570 - 200 * eyesy.knob1));
