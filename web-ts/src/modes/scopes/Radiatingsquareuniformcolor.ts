...
export class Radiatingsquareuniformcolor implements Mode {
  private xr = 1280;
  private yr = 720;
  private sqmover: LFO;
  private adjust1: LFO;
  ...
  draw(canvas: Canvas, eyesy: EYESY): void {
    ...
    for (let i = 0; i < 100; i++) {
      const width = Math.floor(eyesy.knob2 * (this.xr * 0.012)) + 1;
      ...
      // Lines
      if (i < 25) {
        let x0 = (((490 + adjuster1 * i) * this.xr) / this.xr) % this.xr;
      ...
      if (i >= 25 && i < 50) {
        ...
        let y0 = (((510 + adjuster1 * i) * this.yr) / this.yr) % this.yr;
        const y1 = y0 + Math.floor(audioScaled);
        canvas.line([x, y0], [x + angle, y1], color, width);
      }
      if (i >= 50 && i < 75) {
        let x0 = (((790 + adjuster1 * i) * this.xr) / this.xr) % this.xr;
        const x1 = x0 + Math.floor(audioScaled);
      ...
      if (i >= 75 && i < 100) {
        ...
        let y0 = (((210 + adjuster1 * i) * this.yr) / this.yr) % this.yr;
