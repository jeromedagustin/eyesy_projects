 ...
 */
export class Nestedellipsesoutlines implements Mode {
  private xr = 1280;
  private yr = 720;
  ...
  draw(canvas: Canvas, eyesy: EYESY): void {
    ...
    const count = Math.floor(eyesy.knob3 * 99) + 1;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const color = eyesy.color_picker_lfo(eyesy.knob4, 0.008);
      ...
      // Draw the ellipse outline with y_offset
      const lineWidth = Math.floor(eyesy.xres * 0.00625); // Equivalent to Python's 8
      canvas.ellipse(
