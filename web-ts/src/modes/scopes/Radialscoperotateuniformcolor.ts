 ...
 */
export class Radialscoperotateuniformcolor implements Mode {
  private xr = 1280;
  private yr = 720;
  private xr2 = 0;
  private yr2 = 0;
  ...
  private seg(canvas: Canvas, eyesy: EYESY, i: number, angle: number): void {
    ...
    const R = R1 + ((eyesy.audio_in[i] || 0) / ((this.x800 * 20 / (R1 + 1)) + 1));
  ...
  draw(canvas: Canvas, eyesy: EYESY): void {
    ...
    if (eyesy.knob1 >= 0.48 && eyesy.knob1 <= 0.52) {
      ...
      rotationIncrement = (eyesy.knob1 - 0.5) * 20; // Map knob1 value from 0-1 to -1 to 1
    }
