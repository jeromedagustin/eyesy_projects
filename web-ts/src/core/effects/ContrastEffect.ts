/**
 * Contrast Effect - Contrast adjustment
 */
...
export class ContrastEffect implements Effect {
  name = 'contrast';
  enabled = false;
  intensity = 0.0; // 0.0 = no contrast, 1.0 = maximum contrast
  ...
  constructor(renderer: THREE.WebGLRenderer, width: number, height: number) {
    this.createContrastMaterial(width, height);
  }

  private createContrastMaterial(width: number, height: number): void {
    const vertexShader = `
      varying vec2 vUv;
      void main() {
        vUv = uv;
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Contrast adjustment (intensity maps to contrast factor)
        float contrast = 1.0 + (intensity - 0.5) * 2.0; // -1.0 to 1.0 range
  ...
  apply(renderer: THREE.WebGLRenderer, inputTexture: THREE.Texture | null, width: number, height: number): THREE.Texture | null {
    if (!inputTexture || !this.material || !this.scene || !this.camera || !this.renderTarget) {
      console.warn('[ContrastEffect] Cannot apply: missing dependencies');
      return inputTexture;
    }
