/**
 * Sepia Effect - Vintage brown tone effect
 */
import { Effect } from '../EffectManager';
import * as THREE from 'three';

export class SepiaEffect implements Effect {
  name = 'sepia';
  enabled = false;
  intensity = 0.0; // 0.0 = no sepia, 1.0 = full sepia
  ...
  constructor(renderer: THREE.WebGLRenderer, width: number, height: number) {
    this.createSepiaMaterial(width, height);
  }

  private createSepiaMaterial(width: number, height: number): void {
    const vertexShader = `
      varying vec2 vUv;
      void main() {
        vUv = uv;
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        gl_FragColor = vec4(mix(color.rgb, sepiaColor, intensity), color.a);
      }
    `;
  ...
  apply(renderer: THREE.WebGLRenderer, inputTexture: THREE.Texture | null, width: number, height: number): THREE.Texture | null {
    if (!inputTexture || !this.material || !this.scene || !this.camera || !this.renderTarget) {
      console.warn('[SepiaEffect] Cannot apply: missing dependencies');
      return inputTexture;
    }
