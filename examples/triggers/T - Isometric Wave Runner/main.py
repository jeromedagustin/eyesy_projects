...
def setup(screen, eyesy) :
    ...
    for filepath in sorted(glob.glob(eyesy.mode_root + '/Images/*.png')):
        ...
        images.append(img)
    patterns = generate_pattern(75, screen, eyesy)
def generate_pattern(TILEWIDTH_HALF, screen, eyesy):
    ...
    max_range = int(max(1, 570-200*eyesy.knob1))
...
def draw(screen, eyesy):
    ...
    l=int((32)+(32) *(math.sin(time.time()*(.4+1-eyesy.knob1))))
    ...
    for p, pattern in enumerate(patterns):
        ...
        # Draw only if within the visible range
        if(l-64*eyesy.knob4-1) <= p <= (l+64*eyesy.knob4+1):
            pic = images[i]
